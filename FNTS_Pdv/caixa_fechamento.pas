...
type
  ...
  //  qrFechamento: TIBCQuery;
    ...
    qrSuprimentoSangria: TUniQuery;
    qrOperadorVendedor: TUniQuery;
...
var
  ...
  begin
    ...
    cdsDados.Insert;
    cdsDadoslinha.AsInteger := a;
    cdsDadosusuario.AsString   := '  Historico de ';
//    cdsDadosDescricao.AsString := ' Historico de ' + lTitulo;
    cdsDadosDescricao.AsString :=  lTitulo;
    cdsDadosnegrito.AsString := 'S';
    cdsDados.Post;
    Inc(a);
    cdsDados.Insert;
    cdsDadoslinha.AsInteger := a;
    cdsDadosusuario.AsString   := '   Operador';
    cdsDadosDescricao.AsString := '   Horário' + lDescricao;
...
begin
  ...
  begin
    for i := 0 to GridFechamento.RowCount -1 do
    begin
      Inc(a);
      if GridFechamento.Cell[3,i].FontStyle = [fsBold] then
      begin
      ...
      if (GridFechamento.Cells[2,i] <> 'Resumo - Dinheiro em Caixa') then
      begin
        cdsDadosusuario.AsString   := GridFechamento.Cells[1,i];
        cdsDadosDescricao.AsString := GridFechamento.Cells[2,i];
      end
      else
      begin
        cdsDadosusuario.AsString   :=  'Resumo   -';
        cdsDadosDescricao.AsString := 'Dinheiro em Caixa ';
      end;
...
//                  'SELECT DATA, HORA, DESCRICAO, VALOR FROM NAO_FISCAL WHERE ECF = :CAIXA AND DESCRICAO = ''SANGRIA'' AND DATA = :DATA AND (:TIPO = 0 OR :TIPO = 2)');
    ...
    query.SQL.Add('SELECT ADM.INFO1 AS OPERADOR,  NF.DATA, NF.HORA, NF.DESCRICAO, NF.VALOR');
    ...
    begin
      ...
      begin
        ...
        cdsDados.Insert;
        cdsDadoslinha.AsInteger    := a;
        cdsDadosusuario.AsString   := query.FieldByName('OPERADOR').AsString;
...
begin
  ...
  qrOperadorVendedor.Close;
  qrOperadorVendedor.SQL.Clear;
  qrOperadorVendedor.SQL.Add('select distinct(cp.cod_vendedor) as CodOperador,');
  qrOperadorVendedor.SQL.Add('adm.info1 as operador');
  qrOperadorVendedor.SQL.Add('from cupom cp');
  qrOperadorVendedor.SQL.Add('join cupom_forma cpf on (cpf.cod_cupom = cp.codigo)');
  qrOperadorVendedor.SQL.Add('join adm on (adm.codigo = cp.cod_vendedor)');
  qrOperadorVendedor.SQL.Add('where cp.cancelado <> 1');
  qrOperadorVendedor.SQL.Add('and cp.DATA ' + iif(AdvGlowButton1.Enabled, '+ cp.hora >') + '= :data');
  qrOperadorVendedor.ParamByName('data').AsDateTime := FDataFechamento;
  qrOperadorVendedor.Open;
  qrOperadorVendedor.First;

  while not qrOperadorVendedor.Eof do
  begin
    codOperador := qrOperadorVendedor.fieldbyname('CodOperador').AsString;

    qrFechamento.close;
    qrFechamento.sql.clear;
    qrFechamento.sql.add('select CodOperador,');
    ...
    qrFechamento.sql.add('    adm.info1 as operador,');
    qrFechamento.sql.add('    cupom_forma.FORMA as forma,');
    qrFechamento.sql.add('    cupom_forma.VALOR as valor');
    qrFechamento.sql.add('  from cupom');
    qrFechamento.sql.add('    join adm on adm.codigo = cupom.cod_vendedor');
    qrFechamento.sql.add('    join cupom_forma on cupom_forma.cod_cupom = cupom.CODIGO');
    qrFechamento.sql.add('  where cupom.cancelado <> 1');
    qrFechamento.sql.add('  and cupom.DATA ' + iif(AdvGlowButton1.Enabled, '+ cupom.hora >') + '= :data');
    qrFechamento.sql.add('  and cupom.cod_vendedor = :codvendedor ');
    if not bMovCaixa then
      qrFechamento.sql.add('       and cupom.naofisc <> ' + QuotedStr('S'));
    ...
    qrFechamento.Params.ParamByName('data').AsDatetime       := FDataFechamento;
    qrFechamento.Params.ParamByName('codvendedor').AsInteger := qrOperadorVendedor.FieldByName('CodOperador').AsInteger;
    qrFechamento.Open;
    qrFechamento.First;
...
//    GridFechamento.AddRow(1);
    ...
    qrFechamento.sql.add('sum(Valor) as total');
    qrFechamento.sql.add('from');
    qrFechamento.sql.add(' (select nf.codvendedor as CodOperador,');
    qrFechamento.sql.add('    adm.info1 as operador,');
    qrFechamento.sql.add('    nf.descricao as forma,');
    qrFechamento.sql.add('    nf.valor');
    qrFechamento.sql.add('  from nao_fiscal nf');
    qrFechamento.sql.add('    join adm on (adm.codigo = nf.codvendedor)');
    qrFechamento.sql.add('  where nf.data ' + iif(AdvGlowButton1.Enabled, '+ nf.hora >') + '= :data');
    qrFechamento.sql.add('  and nf.codvendedor =:cod ');
    qrFechamento.sql.add('  and nf.INDICE <> ''RG''  ');
    qrFechamento.sql.add(') as tmp');
    qrFechamento.sql.add('  group by  CodOperador,  Operador,  Forma');
    qrFechamento.sql.add('  order by codoperador');
    qrFechamento.Params.ParamByName('data').AsDatetime := FDataFechamento;
    qrFechamento.Params.ParamByName('cod').AsInteger   := StrToInt(codOperador);
    qrFechamento.Open;
    qrFechamento.First;

    if qrFechamento.RecordCount > 0 then
    begin
      while not qrFechamento.Eof do
      begin
        GridFechamento.AddRow(1);
